  ...
  const resetFilters=()=>{
    for(let obj  in filters[filterName]){
      if(filters[filterName][obj].keyword){
        setFilterValues({...filterValues,[filters[filterName][obj].keyword]:null});
      }
      if(filters[filterName][obj].max_keyword){
        setFilterValues({...filterValues,[filters[filterName][obj].max_keyword]:null});
      }
      if(filters[filterName][obj].min_keyword){
        setFilterValues({...filterValues,[filters[filterName][obj].min_keyword]:null});
      }
...
console.log(formIsOpen);
    ...
    return(
     ...
     >
     <List>
        <ListItem sx={{flexDirection:"row-reverse"}} disablePadding>
        ...
        {filters[filterName].map((filter)=>{
            return(
              <>
                    ...
                    <ListItem>
                    {filter.type==="text"?(<TextForm hid={hid} handleParentClose={handleClose}isOpen={formIsOpen[filter.name]} onClose={handleFormClose} filter={filter} filterValues={filterValues} setFilterValues={setFilterValues}/>):filter.type==="number"?(<NumberForm hid={hid} isOpen={formIsOpen[filter.name]} onClose={handleFormClose} filter={filter}  handleParentClose={handleClose} setFilterValues={setFilterValues}  filterValues={filterValues}/>):(<ChoiceForm hid={hid} isOpen={formIsOpen[filter.name]} onClose={handleFormClose} filter={filter} setFilterValues={setFilterValues} handleParentClose={handleClose}  filterValues={filterValues}/>)}
                    </ListItem>
                    </>
            )
        })}
